...
const BerandaPage = () => {
  const [isLoading, setIsLoading] = useState(true); // State untuk mengontrol tampilan loader
  const navigate = useNavigate();

  useEffect(() => {
    // Contoh: Ambil data atau lakukan pekerjaan yang memerlukan waktu
    setTimeout(() => {
      setIsLoading(false); // Setelah proses loading selesai, atur isLoading menjadi false
    }, 2000); // Ganti angka 2000 dengan waktu yang sesuai dengan kebutuhan Anda
  }, []);

  return (
    <>
      {isLoading ? (
        <Loader /> // Tampilkan loader selama isLoading bernilai true
      ) : (
